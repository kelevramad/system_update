...
class RunCommand:
	...
	def execute(self, args: Any, app_ctx: 'SystemUpdateApp') -> None:
		...
		if isinstance(profile, str) and profile:
			...
			configure_network(app_ctx.settings.get('network', {}), app_ctx.config.config_dir)
			...
			register_plugin_command_builders(updater_command_builders(app_ctx.plugins))
			try:
				if app_ctx.history_db:
					app_ctx.history_db.close()
			except Exception:
				pass
			app_ctx.history_db = HistoryDatabase(Path(app_ctx.config.config_dir) / 'history.db')
		...
		if args.source:
			valid, invalid = _partition_sources(args.source, app_ctx.plugin_sources)
			if invalid:
		...
		if apps is None and not no_cache and cache_enabled:
			...
			if cached:
				...
				if app_ctx._include_sources:
					missing = app_ctx._cache_missing_sources(app_ctx._include_sources)
					if pip_stale and 'pip' in app_ctx._include_sources:
						missing.add('pip')
					if missing:
						apps = app_ctx._scan_missing_and_merge(cached, missing)
					else:
						apps = [a for a in cached if a.source.lower() in app_ctx._include_sources]
						app_ctx._maybe_prefetch_cache(cached, app_ctx._include_sources)
						console.print(
					...
					if missing and app_ctx.settings.get('cache', {}).get(
				...
				apps = app_ctx._scan_missing_and_merge([], set(app_ctx._include_sources))
			else:
		...
		if apps is None:
			...
			app_ctx.checker.check_all_updates(
				apps,
				max_workers=app_ctx.settings.get('performance', {}).get('max_workers', 4),
			...
			for app in apps:
				for finding in app.security_findings:
					app_ctx.vuln_history.record_vulnerability(app, finding, scan_id)

			scanned_sources = app_ctx._scanned_sources_label(args)
			scan_time = time.time() - start_time
			app_ctx.history_db.record_scan(apps, scan_id, scanned_sources, scan_time)
			...
			if args.no_cache:
				...
				app_ctx._save_cache_with_context(
					apps,
					app_ctx._fresh_scan_sources(args, apps),
					scan_time=scan_time,
				)
				console.print(
		...
		if updates or vulnerable:
			...
			total_count = app_ctx._print_available_updates_summary(updates, security_updates)
			...
			if args.show_commands:
				...
				commands = []
				for app in security_updates + [
					u for u in updates if u.update_status != UpdateStatus.VULNERABLE
				]:
				...
				if commands:
					console.print()
					console.print('[bold cyan]💻 Manual Update Commands:[/bold cyan]')
					cmd_text = '\n'.join(' '.join(c) for c in commands)
					console.print(
						Panel(
							cmd_text,
							title='Copy & paste to run manually',
							border_style='cyan',
							expand=False,
						)
					)
					...
					console.print(
						'\n[yellow]⚠️ No manual update commands found or supported for these packages.[/yellow]'
					)
		...
		if isinstance(export_format, str) and export_format:
			...
			if export_format == 'html':
				...
				template_path = args.html_template or (
					app_ctx.settings.get('report', {}).get('template_path') or None
				)
