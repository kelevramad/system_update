	...
	def _handle_remote(self, args: Namespace) -> None:
		...
		if action == 'remove':
			...
			if ok:
				...
				console.print(f'[yellow]No host named[/yellow] [bold]{name}[/bold]')
			return
		...
		results = self._run_remote_with_progress(hosts, cmd, timeout, verbose, debug)
			...
			def _on_done(host, result) -> None:
				...
				if verbose:
					...
					if result.stderr:
						...
						progress.console.print(
							'[red]stderr:[/red] '
							+ ('\n  ' + '\n  '.join(err_tail) if err_tail else '(empty)')
						)
