...
class PluginRegistry:
    ...
    def error(self, path: Path | str, exc: Exception) -> None:
        ...
        self.errors.append(PluginLoadError(
...
def load_plugins(config: Any) -> PluginRegistry:
    ...
    * The ``--no-plugins`` CLI flag bypasses the loader entirely.
    ...
    require_allowlist = bool(plugin_settings.get(
...
def dispatch_notifiers(
    ...
    for notifier in registry.notifiers.values():
        if not notifier.enabled:
            continue
        try:
            _call_notifier(notifier.notify, event, title,
...
def _coerce_app(item: AppInfo | Dict[str, Any], default_source: str) -> AppInfo:
    ...
    status = item.get('update_status', item.get(
    ...
    return AppInfo(
        ...
        latest_version=str(
            item.get('latest_version', item.get('latestVersion', '')) or ''),
        ...
        security_findings=list(
            item.get('security_findings', item.get(
                'securityFindings', [])) or []
        ),
    )
