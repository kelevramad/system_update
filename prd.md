...
**Document Version:** 8.14.2
**Last Updated:** July 2, 2026
**Author:** Kelevra Mad
**Based On:** `src/system_update/` package (v8.14.2)
...
| 8.13.0 | May 2026 | Hardening 5 — quality and observability: added `_RedactionFilter` + `get_logger(name, **context)` helpers in `config.py` so every log handler scrubs `password=`, `token=`, `secret=`, `api_key=`, `Authorization:`, `Bearer …`, and `winrs -p:…` substrings before they reach disk or stderr; structured context fields render as `[k=v …]` prefix on emitted records. New tests cover the redaction filter, the context adapter, and end-to-end log-file output. New `tests/test_logging.py`, plus targeted coverage for `cache.is_source_valid`, `_apply_smart_sources_filtering`, and `aggregate_scans` malformed-payload paths. Added `hypothesis` dev dependency and `tests/test_scanner_fuzz.py` with property-based fuzz over winget/scoop/choco/bun/rust/yarn parsers (marked `slow`, deselected from the default suite via `-m 'not slow'`). Added `.github/workflows/windows-smoke.yml` — Windows runner installs uv, runs scan → JSON validate → snapshot list → HTML export, uploads scan/log artifacts on failure. Full verification is 671 passed, 4 skipped, 9 deselected (slow) plus clean Ruff and Pyright |
| 8.12.0 | May 2026 | Hardening 4 — performance: parallelized GitHub Advisory queries via `ThreadPoolExecutor` (workers from `security.github_workers`, default 4); compiled scanner regexes at module level (`bun`, `rust`, `yarn`, `path`); memoized `cache.json` reads by mtime so `is_valid` + `is_source_valid` + `load` share one parse; honored the real `tick_interval` in `remote.execute_many` (no more hardcoded 1s spin); indexed pip apps by lower-cased name to remove the O(N·M) `next(...)` lookup; added retry/backoff on transient HTTP errors (429/5xx, connection errors) in `network.fetch_json` honoring `Retry-After` with exponential-backoff + jitter, configurable via `network.retry_max_attempts` / `retry_base_seconds` / `retry_max_seconds`. New tests cover memoization (incl. mtime + write invalidation), retry/backoff, pip indexed lookup, GitHub parallel pool, and large-`tick_interval` fast completion; full verification is 645 passed, 4 skipped plus clean Ruff and Pyright |
...
| 8.7.0 | May 2026 | Hardening 2.2 — Windows encoding and parsing: new `decode_command_output()` captures subprocess stdout/stderr as raw bytes and decodes through a UTF-8 → UTF-16-LE BOM → Windows OEM code page → CP1252 fallback chain (`errors='ignore'` is gone), so accented vendor names like `Café`, `Software Müller`, and pt-BR/DE/ES titles round-trip without mojibake. New `scanners/_winget_table.py` centralizes a locale-agnostic winget parser — the table parser anchors on the dashed-separator line winget always emits and computes column offsets from the *header's* whitespace pattern, so en-US, pt-BR (`Nome / ID / Versão / Disponível / Origem`), and de-DE (`Name / ID / Version / Verfügbar / Quelle`) outputs all parse identically. A JSON fast-path (`winget --format json`, winget ≥ 1.7) is tried first. New `parse_iso_utc()` helper replaces every `fromisoformat(s.replace('Z', ''))` site (4 in `cache.py`, 1 in `history.py`); cache freshness checks now use timezone-aware UTC arithmetic so UTC-Z entries no longer flap between valid and invalid based on the local clock |
| 8.6.0 | May 2026 | Hardening 2.1 — concurrency: `HistoryDatabase` and `SnapshotStore` switched from a cached single `sqlite3.Connection` to **per-thread connections** via `threading.local`, with `journal_mode=WAL` + `synchronous=NORMAL` + `foreign_keys=ON` so concurrent readers coexist with one writer. Connections are opened with `check_same_thread=False` so `close()` works from any thread; every Connection is also tracked in a per-instance list so `close()` drains them across threads (eliminates `ResourceWarning: unclosed database` on GC). Snapshot ids gain `microsecond + itertools.count` to prevent collisions when 8+ threads record in the same second. Per-host network rate limiter replaces the process-global RLock + `time.sleep` (which serialized every HTTP call across every host); each host now has its own `_HostLimiter` and the slow `time.sleep` happens **outside** the lock. New `tests/test_concurrency.py` proves 8-thread `record_scan` / `record` with all rows present + unique ids, per-thread connection isolation, WAL active; new `tests/test_network.py` cases prove cross-host calls don't sleep when both slots are free and that `time.sleep` doesn't run while the per-host lock is held |
| 8.5.2 | May 2026 | Safe Parsing (Hardening 1.5): remote WinRM JSON stdout is capped by `remote.max_response_bytes` (default 10 MiB) before parsing; oversized and malformed JSON-looking responses from `winrs` and `pywinrm` become per-host errors. XML export is documented as generation-only with regression coverage asserting no XML parser imports in the export path |
| 8.5.1 | May 2026 | Test patch — cover the Windows ACL hardening path that v8.5.0 introduced. Three new tests in `tests/test_secure_paths.py`: a unit test asserts `secure_write` shells out to `icacls <path> /inheritance:r /grant <USER>:(F)` twice (temp file before rename + destination as belt-and-suspenders); an integration test runs `icacls` for real and asserts no `Everyone:` / `BUILTIN\Users:` / `NT AUTHORITY\Authenticated Users:` ACE survives the inheritance reset; and the third covers `harden_existing_file`. All gated `@pytest.mark.skipif(platform.system() != 'Windows')`. No runtime behavior change |
| 8.5.0 | May 2026 | Hardening 1.4 — file permissions: new `utils.data_dir()` accessor centralizes `~/.system_update` (honors `SYSTEM_UPDATE_HOME`, creates the dir with mode `0o700` on POSIX) and replaces 8 hardcoded `Path.home() / '.system_update'` sites across `config.py`, `history.py`, `snapshots.py`, `remote.py`, and `plugins.py`. New `utils.secure_write()` helper performs atomic write-then-replace with `0o600` permissions on POSIX and an `icacls /inheritance:r /grant USER:(F)` user-only ACL on Windows; the temp file gets the restrictive ACL **before** any data hits disk so the file never exists with looser permissions. Failed writes wipe the temp and leave the previous content intact. Migrates 6 write sites (`cache.json`, `config.json`/`yaml`, exported profiles, `inventory.json`, `vulnerability_history.json`, `api_cache.json`). New `utils.harden_existing_file()` hook applies the same ACL to SQLite databases (`history.db`, `snapshots.db`) once `sqlite3.connect` has created them |
| 8.4.1 | May 2026 | Quality patch — clear the entire Pyright baseline (80 → 0 errors). Source-side: introduce a typed `conn` `@property` over `Optional[sqlite3.Connection]` in `history.py` (13 errors) and the same shape in `snapshots.py` (2); narrow `sys.stdout.reconfigure` with `isinstance(_, io.TextIOWrapper)`; coerce `cache._app_from_dict` `name`/`version` through `str(... or '')`; switch `checkers.__init__` parameter types from invariant `Dict` to covariant `Mapping`; tighten `export.py`/`app.py` boundaries to `Optional[str]`; fix `_format_size` int/float drift, `_latest_cell` return type, and `_cvss_display` narrowing in `ui/system.py`; `os.geteuid` via `getattr` for Windows; `RemoteResult.parsed: Optional[Any]` to match its docstring. Test-side: assert-then-access for `snapshots.get`, `build_rollback_command`, `cache.load`, `task_status` (43 errors); `_as_config()` cast helper for `setup_logging(SimpleNamespace)` (6); `# type: ignore` on three documented test-only patterns (monkeypatch, `HTTPError` headers init, dynamic module attrs). `CommandError.classify` widened to `BaseException` so the existing `KeyboardInterrupt` test typechecks. No runtime behavior change |
